...
class ImapCredentialsController {
    ...
    private DbController $cl;

    public function __construct(DbController $cl, LoggerInterface $logger) {
    ...
    error_log("=== IMAP DEBUG ===");
    error_log("Host: " . $account['imap_host']);
    error_log("Port: " . $account['imap_port']);
    error_log("Username: " . $account['imap_username']);
    error_log("Password length: " . strlen($account['imap_password']));
    error_log("SSL: " . ($account['use_ssl'] ? 'YES' : 'NO'));
    
    try {
        // 1. Socket-Test ZUERST
        $socket = @fsockopen($account['imap_host'], $account['imap_port'], $errno, $errstr, 10);
        if (!$socket) {
            ...
            return [
                'success' => false,
                'message' => "Kann Server nicht erreichen: $errstr (Port {$account['imap_port']})"
            ];
        }
        fclose($socket);
        error_log("Socket-Test: OK");
        
        // 2. Mailbox-String erstellen
        $mailbox = sprintf(
        ...
        error_log("Mailbox: $mailbox");
        
        // 3. Timeouts setzen
        imap_timeout(IMAP_OPENTIMEOUT, 30);
        imap_timeout(IMAP_READTIMEOUT, 30);
        
        // 4. Alte Fehler löschen
        @imap_errors();
        @imap_alerts();
        
        // 5. IMAP-Verbindung (NUR 1 Versuch!)
        error_log("Versuche IMAP-Verbindung...");
        ...
        if ($imap === false) {
            ...
            error_log("IMAP FAILED: $error");
            if ($errors) {
                error_log("All errors: " . print_r($errors, true));
            }
            
            // Spezifische Fehlerbehandlung
            if (strpos($error, 'Too many login failures') !== false) {
                return [
                    'success' => false,
                    'message' => 'Account temporär gesperrt. Bitte warte 30-60 Minuten.'
                ];
            }
            ...
            if (strpos($error, 'authentication failed') !== false) {
                return [
                    'success' => false,
                    'message' => 'Authentifizierung fehlgeschlagen. Prüfe Username (muss vollständige E-Mail sein) und Passwort. Stelle sicher, dass IMAP in den WEB.DE Einstellungen aktiviert ist.'
                ];
            }
            
            if (strpos($error, 'Couldn\'t open stream') !== false) {
                return [
                    'success' => false,
                    'message' => 'Kann keine Verbindung zum Server aufbauen. Firewall oder OpenSSL-Problem?'
                ];
            }
        ...
        return [
            'success' => true,
            'message' => 'Verbindung erfolgreich! (' . $check->Nmsgs . ' Nachrichten)'
        ];
        
    } catch (Exception $e) {
        error_log("EXCEPTION: " . $e->getMessage());
        
        $this->logger->error('IMAP Exception', [
            'error' => $e->getMessage(),
